let defaultApiUrl = 'http://openapi.texhr.cn/gateway/'

// H5 本地开发时改用 webpack-dev-server 反向代理，避免浏览器跨域限制。
// #ifdef H5
if (process.env.NODE_ENV === 'development') {
	defaultApiUrl = '/proxy/texhr-openapi/gateway/'
}
// #endif
...
export function configureRequest({ apiUrl } = {}) {
	if (apiUrl) requestConfig.apiUrl = apiUrl
}
...
function getAuthFromCookie() {
	const personalCookie = getCookie(AUTH_COOKIE_KEYS.PERSONAL)
	if (!personalCookie) return { PersonId: '', Token: '' }

	try {
		const personal = JSON.parse(personalCookie)
		return {
			PersonId: personal.personId === undefined || personal.personId === null
				? ''
				: String(personal.personId),
			Token: typeof personal.token === 'string' ? personal.token : ''
		}
	} catch (error) {
